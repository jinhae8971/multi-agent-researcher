...
stance: {stances_str} 중 하나"""
    ...
    def meta_review(
        ...
        collected_at = research_data.get("collected_at", "시각 미상")
...
[⚠️ 데이터 정합성 검증 — 최종 점검]
- Author 및 Reviewer의 분석에서 소스 데이터에 없는 구체적 수치(지수 포인트, 환율, 금리 등)가 사용되었는지 확인하세요.
- 학습 데이터에 기반한 과거 수치(예: 과거 지수 수준)가 현재 데이터인 것처럼 사용된 경우, summary에서 이를 지적하고 해당 수치를 제외한 판단을 내리세요.
- 소스에서 확인 불가능한 수치는 최종 판단에 포함하지 마세요.

반드시 아래 JSON으로만 응답:
{{
  "final_stance": "{self.stance_values[0]}",
  "confidence_score": 68,
  "summary": "Meta-Reviewer 종합 근거 설명 (300자 이상, 소스 기반 수치만 인용)",
  "key_insights": ["핵심 인사이트 1", "핵심 인사이트 2", "핵심 인사이트 3"],
  "risk_factors": ["리스크 1", "리스크 2"],
  "action_items": ["실행 제안 1", "실행 제안 2"],
