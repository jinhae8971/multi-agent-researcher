...
[⚠️ 데이터 정합성 — 최우선 규칙]
- 제공된 소스 데이터에 명시된 수치·팩트만 사용하세요.
- 당신의 학습 데이터(training data)에 기반한 과거 수치를 절대 사용하지 마세요.
- 주가지수, 환율, 금리 등 시계열 수치는 소스에서 직접 인용한 것만 사용하세요.
- 소스에 없는 구체적 수치를 추측하거나 생성하지 마세요.
- 이 규칙을 위반하면 분석의 신뢰성이 심각하게 훼손됩니다.
...
- 제공된 소스 데이터에 명시된 수치·팩트만 사용하세요.
- 당신의 학습 데이터(training data)에 기반한 과거 수치를 절대 사용하지 마세요.
- 주가지수, 환율, 금리, 시가총액 등 시계열 수치는 소스에서 직접 인용한 것만 사용하세요.
- 소스에 없는 구체적 수치(예: 지수 포인트, 특정 가격)를 추측하거나 생성하지 마세요.
- 특히 "~선 지지/저항" 같은 구체적 가격 수준은 소스에서 확인된 경우만 언급하세요.
- 이 규칙을 위반하면 분석의 신뢰성이 심각하게 훼손됩니다.
...
[⚠️ 데이터 정합성 검증 — 핵심 리뷰 항목]
- Author가 소스에 없는 구체적 수치(지수, 가격, 환율 등)를 사용했는지 반드시 확인하세요.
- Author의 학습 데이터(training data)에서 온 구시대 수치가 있으면 key_concerns에 명시하세요.
- 특히 지수 포인트, 금리 수준, 환율 등 시계열 데이터의 시점 정합성을 검증하세요.
- 데이터 정합성이 무너진 분석은 반드시 disagree 판정을 내려야 합니다.
...
[⚠️ 데이터 사용 규칙 — 반드시 준수]
- 위 소스 데이터에 명시된 수치·팩트만 사용하세요.
- 당신의 학습 데이터(training data)에서 기억나는 과거 수치를 절대 사용하지 마세요.
- 주가지수(코스피, S&P500 등), 환율, 금리 등의 구체적 수치는 소스에서 직접 인용한 경우만 사용하세요.
- 소스에 없는 수치를 언급해야 할 경우, "소스 미확인"이라고 명시하세요.
- 위반 시 분석의 신뢰성이 완전히 무너집니다.

[분석 가이드 — Author 모드]
1. 먼저 소스 데이터의 핵심 팩트를 정리하세요 (출처 URL 병기).
...
반드시 아래 JSON으로만 응답:
{{
  "analysis": "500자 이상 종합 분석 (추론 과정 포함, 찬반 근거 모두 포함, 소스 데이터만 인용)",
...
- 위 소스 데이터에 명시된 수치·팩트만 사용하세요.
- 당신의 학습 데이터(training data)에서 기억나는 과거 수치를 절대 사용하지 마세요.
- 주가지수, 환율, 금리 등의 구체적 수치는 소스에서 직접 인용한 경우만 사용하세요.
- 소스에 없는 수치를 언급해야 할 경우, "소스 미확인"이라고 명시하세요.
...
반드시 아래 JSON으로만 응답:
{{
  "analysis": "300자 이상 분석 (소스 데이터 기반 구체적 근거 포함)",
  "key_points": ["핵심 인사이트 1", "핵심 인사이트 2", "핵심 인사이트 3"],
  "confidence_score": 75,
  "stance": "{self.stance_values[0]}"
}}
...
[⚠️ 데이터 정합성 검증 — 핵심 리뷰 항목]
- Author가 위 소스 데이터에 없는 구체적 수치(지수, 가격, 환율, 금리 등)를 사용했는지 반드시 확인하세요.
- 학습 데이터(training data)에서 온 구시대 수치가 있으면 key_concerns에 반드시 명시하세요.
- 데이터 정합성이 무너진 분석은 반드시 disagree 또는 partial 판정을 내려야 합니다.

[리뷰 가이드]
1. Author의 분석이 당신의 전문 분야에서 타당한가?
2. Author가 소스에 없는 수치를 사용하거나 과거 데이터를 혼용하지 않았는가?
3. Author가 놓친 중요한 관점이나 데이터가 있는가?
4. 분석의 논리적 강점과 약점은 무엇인가?
5. 수정이 필요하다면 구체적으로 어떤 부분인가?

반드시 아래 JSON으로만 응답:
{{
  "decision": "agree",
  "confidence": 4,
  "justification": "200~400자 판단 근거 (데이터 정합성 검증 결과 포함)",
